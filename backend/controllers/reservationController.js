...
const generateReport = async (req, res) => {
  try {
    ...
    reservations.forEach((reservation, index) => {
      const driverName = reservation?.driverId?.name || 'N/A';
      const driverEmail = reservation?.driverId?.email || 'N/A';
      const contact = reservation?.contactNumber || 'N/A';
      const vehicleNumber = reservation?.vehicleNumber || 'N/A';
      const vehicleType = reservation?.vehicleType || 'N/A';
      const slotId = reservation?.parkingSlotId?.slotId || 'N/A';
      const slotLocation = reservation?.parkingSlotId?.location || 'N/A';
      const reservedDate = reservation?.reservedDate ? new Date(reservation.reservedDate).toLocaleDateString() : 'N/A';
      const entryTime = reservation?.entryTime ? new Date(reservation.entryTime).toLocaleString() : 'N/A';
      const exitTime = reservation?.exitTime ? new Date(reservation.exitTime).toLocaleString() : 'N/A';
      const status = reservation?.status || 'N/A';

      reportContent += `${index + 1}. Reservation ID: ${reservation.reservationId || 'N/A'}\n`;
      reportContent += `   Driver: ${driverName} (${driverEmail})\n`;
      reportContent += `   Contact: ${contact}\n`;
      reportContent += `   Vehicle: ${vehicleNumber} (${vehicleType})\n`;
      reportContent += `   Parking Slot: ${slotId} - ${slotLocation}\n`;
      reportContent += `   Reserved Date: ${reservedDate}\n`;
      reportContent += `   Entry Time: ${entryTime}\n`;
      reportContent += `   Exit Time: ${exitTime}\n`;
