...
const TrackingStatus = () => {
  ...
  useEffect(() => {
    // Determine logged-in driver ID
    try {
      const storedUser = localStorage.getItem('userData');
      if (storedUser) {
        const user = JSON.parse(storedUser);
        if (user && user._id) {
          setCurrentDriverId(user._id);
        }
      }
    } catch (_) {}
  ...
  useEffect(() => {
    // Force filter to current driver's ID; ignore the manual search when logged in
    const driverIdFilter = currentDriverId || searchDriverId;
    ...
    if (!driverIdFilter || driverIdFilter.trim() === '') {
      ...
      const filtered = validReservations.filter(reservation => {
        ...
        return driverIdToSearch.toLowerCase().includes(driverIdFilter.toLowerCase());
      });
      setFilteredReservations(filtered);
    }
  }, [searchDriverId, reservations, currentDriverId]);
  ...
  const fetchAllReservations = async () => {
    try {
      ...
      let response;
      if (currentDriverId) {
        response = await reservationAPI.getReservationsByDriver(currentDriverId);
      } else {
        response = await reservationAPI.getAllReservations();
      }
  ...
  return (
    <div style={{
      ...
      {/* Search Filter */}
      {/* Search Filter hidden for logged-in user context */}
      {(!currentDriverId || currentDriverId.trim() === '') && (
        <div style={{
